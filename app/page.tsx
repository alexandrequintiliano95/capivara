...
import { GOALS, goalProgress, projectCost, act, advance, clickValue, hireCost, ISLANDS, multiplier, newGame, PRODUCTIONS, productionRate, rate, restoreGame, SAVE_KEY, type Action, type Game } from '@/lib/game';
...
const productionIcons = [Trees, Sprout, ChefHat, Sun, ShoppingBasket, Leaf];
...
export default function Home() {
  ...
  function play(action: Action) {
    ...
    else if (action.type === 'claim') announce('Objetivo concluído! Recompensa na cesta.');
  ...
  return (
    <div className="game-shell">
      ...
      <main id="ilha">
        ...
        <div className="welcome-row"><div><h1>Seu pedacinho de sossego<span>.</span></h1><p>Pequenas colheitas. Grandes amizades.</p></div><span className="island-badge"><Map size={16} /> Ilha {game.island} de {ISLANDS.length}</span></div>
        ...
        <div className="play-layout">
          <section className="island-column" aria-label="Sua ilha">
            ...
            <section className="expansion" aria-labelledby="expand-title">
              ...
              {nextIsland ? <Button className="expand-button" disabled={!ready || game.fruits < nextIsland.cost || residents < nextIsland.residents} onClick={() => play({ type: 'expand' })}><span>Expandir ilha<small><Apple size={13} /> {fmt(nextIsland.cost)}</small></span><ArrowUpRight size={19} /></Button> : <Check className="completed-icon" size={26} />}
            </section>
          ...
          <aside className="management" aria-label="Produção e melhorias">
            ...
            <Tabs defaultValue="production" className="management-tabs"><TabsList className="management-tabs-list" aria-label="Gerenciar a ilha"><TabsTrigger value="production"><Sprout size={17} /> Produção</TabsTrigger><TabsTrigger value="upgrades"><Sparkles size={17} /> Melhorias</TabsTrigger><TabsTrigger value="goals"><Check size={17} /> Objetivos</TabsTrigger></TabsList>
              ...
              <TabsContent value="goals"><div className="section-intro"><h2>Uma ilha cheia de histórias</h2><p>{game.claimed.length} de {GOALS.length} objetivos concluídos. Resgate cada recompensa uma vez.</p></div>{GOALS.map((goal, index) => {
                const progress = goalProgress(game, index); const claimed = game.claimed.includes(index);
                const unit = { units: 'moradores', earned: 'frutas acumuladas', island: 'áreas abertas', upgrades: 'melhorias', project: 'níveis da praça' }[goal.kind];
                return <section className="goal-row" key={goal.name}><h3>{goal.name}</h3><p>{fmt(Math.min(progress, goal.target))} / {fmt(goal.target)} {unit}</p><Progress value={Math.min(100, progress / goal.target * 100)} aria-label={goal.name} /><Button className="hire-button" disabled={!ready || claimed || progress < goal.target} onClick={() => play({ type: 'claim', index })}>{claimed ? 'Resgatado' : `Resgatar ${fmt(goal.reward)} frutas`}</Button></section>;
              })}</TabsContent>
            </Tabs>
            <section className="island-journal"><div><Trees size={22} /><h2>Praça da amizade · {game.project}/20</h2></div><p>Cada nível soma +10% ao bônus de produção e colheita. Invista na praça ou guarde frutas para a próxima área.</p><Button className="hire-button" disabled={!ready || game.project >= 20 || game.fruits < projectCost(game)} onClick={() => play({ type: 'project' })}>{game.project >= 20 ? 'Monumento completo' : `Construir · ${fmt(projectCost(game))} frutas`}</Button></section>
            <div className="island-journal"><div><Sun size={22} /><h2>Devagar também se vai longe.</h2></div><p>{unlocked === 1 ? 'Sua primeira amiga já está colhendo. Toque em “Colher frutas” para ajudar e convide mais capivaras.' : unlocked === 2 ? 'A horta está cheia de vida. A próxima aventura tem cheirinho de comida caseira.' : 'Apiário, feira e santuário esperam por você. Confira os objetivos e prepare a próxima expansão.'}</p></div>
          </aside>
        </div>
